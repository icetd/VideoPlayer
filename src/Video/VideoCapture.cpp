    ...
    while (is_connected) {
        ...
        if (ret < 0) {
            ...
            if (ret == AVERROR(EIO) || ret == AVERROR(ECONNRESET)) {
                LOG(ERRO, "Network error detected, attempting to reconnect...");
                if (!reconnect()) {
                    return false;
                }
                continue; 
            ...
            LOG(ERRO, "av_read_frame failed: %s", errStr);
            return false;
        }
        ...
        if (response < 0) {
            ...
            LOG(ERRO, "Failed to send packet: %s\n", errStr);
        ...
        if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
            ...
            av_strerror(response, errStr, sizeof(errStr));
            LOG(ERRO, "Failed to receive frame: %s\n", errStr);
        ...
        *pts = av_frame->pts;
        ...
        if (!sws_scaler_ctx || av_frame->width != sws_width || av_frame->height != sws_height || av_codec_ctx->pix_fmt != sws_pix_fmt) {
            ...
            if (!sws_scaler_ctx) {
                LOG(ERRO, "Couldn't initialize SWS context\n");
                return false;
            }
