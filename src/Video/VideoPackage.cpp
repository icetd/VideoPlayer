...
void VideoPackage::start(const char* outFilename, int in_width, int in_height)
{
	...
	if (!ofmt_ctx) {
		LOG(ERRO, "Couldn't create out put contex");
		return;
	}
	...
	if (avio_open(&ofmt_ctx->pb, outFilename, AVIO_FLAG_READ_WRITE) < 0) {
		LOG(ERRO, "Open output file failed");
		return;
	}

	video_st = avformat_new_stream(ofmt_ctx, NULL);
	if (!video_st) {
		LOG(ERRO, "allocate output stream failed");
		return;
	}
	...
	if (!pCodec) {
		LOG(ERRO, "Can't find encoder ");
		return;
	}
	int ret;
	if ((ret = avcodec_open2(pCodecCtx, pCodec, NULL)) < 0) {
		LOG(ERRO, "Can't open encoder ");
		perror("error");
		return;
	}
	...
	if(!sws_ctx)
	{
		LOG(ERRO, "Impossible to create scale context for format conversion!");
		return;
	}
...
void VideoPackage::stop()
{
	...
	if(ret < 0)
	{
		LOG(ERRO, "Flushing encoder failed.");
		return ;
	}
